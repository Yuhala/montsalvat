...
public class SimFFT {
    ...
    /**
     * Accuracy check on FFT of data. Make a copy of data, Compute the FFT, then the
     * inverse and compare to the original. Returns the rms difference.
     */
    ...
    /**
     * Our input here is the number of MB (i.e size of the sequence). Please input
     * only powers of 2. The size of the corresponding double sequence will be
     * calculated accordingly. We assume a double is 8 bytes. Pyuhala
     * 
     * @param n
     */
    public static void doFFT(int size) {

        // x MB == Math.power(2,17) * x doubles

        int n = (int) Math.pow(2, 17) * size;
        test(makeRandom(n));

        //System.out.println("Computing fft of a: " + size + " MB double sequence. RMS Error: " + test(makeRandom(n)));
