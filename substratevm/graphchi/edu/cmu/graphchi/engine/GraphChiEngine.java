...
public class GraphChiEngine<VertexDataType, EdgeDataType> {
    ...
    // Logger.getLogger(Logger.GLOBAL_LOGGER_NAME);// ChiLogger.getLogger("engine");
    ...
    /* Metrics */
    // pyuhala: testing in sgx; these cause issues.
    // private final Timer loadTimer =
    // Metrics.defaultRegistry().newTimer(GraphChiEngine.class, "shard-loading",
    // TimeUnit.SECONDS, TimeUnit.MINUTES);
    // private final Timer executionTimer =
    // Metrics.defaultRegistry().newTimer(GraphChiEngine.class, "execute-updates",
    // TimeUnit.SECONDS, TimeUnit.MINUTES);
    // private final Timer waitForFutureTimer =
    // Metrics.defaultRegistry().newTimer(GraphChiEngine.class, "wait-for-future",
    // TimeUnit.SECONDS, TimeUnit.MINUTES);
    // private final Timer initVerticesTimer =
    // Metrics.defaultRegistry().newTimer(GraphChiEngine.class, "init-vertices",
    // TimeUnit.SECONDS, TimeUnit.MINUTES);
    // private final Timer determineNextWindowTimer =
    // Metrics.defaultRegistry().newTimer(GraphChiEngine.class, "det-next-window",
    // TimeUnit.SECONDS, TimeUnit.MINUTES);
     ...
     * @throws IOException
     */
    public GraphChiEngine(int nShards, String baseFilename) throws FileNotFoundException, IOException {
        this.baseFilename = baseFilename;
        this.nShards = nShards;
        // disable logger: pyuhala
        // LogManager.getLogManager().reset();

        System.out.println("BaseFilename is: " + baseFilename);
        loadIntervals();
    ...
    protected void loadIntervals() throws FileNotFoundException, IOException {
        // System.out.println("loadIntervals::baseFilename " + baseFilename);
        intervals = ChiFilenames.loadIntervals(baseFilename, nShards);
    }

    /**
     * Set the memorybudget in megabytes. Default is JVM's max memory / 4. Memory
     * budget affects the number of vertices loaded into memory at any time.
     * 
     * @param mb
     */
    ...
    /**
     * You can instruct the engine to automatically ignore vertices that do not have
     * any edges. By default this is <b>false</b>.
     * 
    ...
    /**
     * For definition of "sliding shards", see
    ...
    protected void initializeSlidingShards() throws IOException {
        ...
        for (int p = 0; p < nShards; p++) {
            String edataFilename = (onlyAdjacency ? null
                    : ChiFilenames.getFilenameShardEdata(baseFilename, edataConverter, p, nShards));
            String adjFilename = ChiFilenames.getFilenameShardsAdj(baseFilename, p, nShards);

            SlidingShard<EdgeDataType> slidingShard = new SlidingShard<EdgeDataType>(edataFilename, adjFilename,
                    intervals.get(p).getFirstVertex(), intervals.get(p).getLastVertex());
    ...
    /**
     * For definition of "memory shards", see
     * http://code.google.com/p/graphchi/wiki/IntroductionToGraphChi
     * 
     * @throws IOException
     */
    protected MemoryShard<EdgeDataType> createMemoryShard(int intervalStart, int intervalEnd, int execInterval) {
        String edataFilename = (onlyAdjacency ? null
                : ChiFilenames.getFilenameShardEdata(baseFilename, edataConverter, execInterval, nShards));
        ...
        MemoryShard<EdgeDataType> newMemoryShard = new MemoryShard<EdgeDataType>(edataFilename, adjFilename,
                intervals.get(execInterval).getFirstVertex(), intervals.get(execInterval).getLastVertex());
    ...
    /**
     * Runs the GraphChi program for given number of iterations. <b>Note:</b> Prior
     * to calling this, you must have set the edge-data and vertex-data converters:
     * setEdataConverter() setVertexDataConverter()
     * 
     ...
     */
    /**
     * pyuhala: fix-me! instrumenter does not handle interfaces correctly using
     * pagerank class directly
     * 
     */
    public void run(String programName, int niters) throws IOException {

        CompressedIO.disableCompression();
        // ------------------------------------ Temp modifs for partitioning
        // -----------------------------------------

        GraphChiProgram<VertexDataType, EdgeDataType> program = (GraphChiProgram<VertexDataType, EdgeDataType>) new Pagerank();

        assert (programName.contains("Pagerank"))
                : "engine runs only pagerank for now; edit run method to create an object for your program";
        ...
        System.out.println("In run methodxxx:this.baseFile: " + this.baseFilename);

        /*
         * int nprocs = 4; if (Runtime.getRuntime().availableProcessors() > nprocs) {
         * nprocs = Runtime.getRuntime().availableProcessors(); }
         * 
         * if (System.getProperty("num_threads") != null) nprocs =
         * Integer.parseInt(System.getProperty("num_threads"));
         * 
         * //logger.info(":::::::: Using " + nprocs + " execution threads :::::::::");
         * 
         * parallelExecutor = Executors.newFixedThreadPool(nprocs); loadingExecutor =
         * Executors.newFixedThreadPool(4);
         */
        // avoiding multithreading at all cost for sgx enclaves: b/c problematic
        ...
        for (int iter = 0; iter < niters; iter++) {
            ...
            for (int execInterval = 0; execInterval < nShards; ++execInterval) {
                ...
                // logger.info((System.currentTimeMillis() - startTime) * 0.001 + "s: iteration:
                // " + iter + ", interval: "
                ...
                while (subIntervalStart <= intervalEn) {
                    ...
                    if (anyVertexScheduled(subIntervalStart, Math.min(intervalEn, subIntervalStart + adjMaxWindow))) {
                        ...
                        if (!autoLoadNext || nextWindow == null) {
                            try {
                                subIntervalEnd = determineNextWindow(subIntervalStart,
                                        Math.min(intervalEn, subIntervalStart + adjMaxWindow));
                            ...
                            try {
                                ...
                                // _timer.stop();
                                // logger.info("Waiting for future task loading took " +
                                // (System.currentTimeMillis() - tf)+ " ms");
                                if (subIntervalStart != next.getSubInterval().getFirstVertex())
                                    throw new IllegalStateException(
                                            "Future loaders interval does not match the expected one! "
                                                    + subIntervalStart + " != "
                                                    + next.getSubInterval().getFirstVertex());
                        ...
                        if (autoLoadNext) {
                            ...
                            if (subIntervalEnd + 1 <= intervalEn) {
                                nextWindow = new FutureTask<IntervalData>(new AutoLoaderTask(
                                        new VertexInterval(subIntervalEnd + 1,
                                                Math.min(intervalEn, subIntervalEnd + 1 + adjMaxWindow)),
                                        execInterval, memoryShard));
                                ...
                                nextWindow = new FutureTask<IntervalData>(new AutoLoaderTask(
                                        new VertexInterval(nextIntervalSt,
                                                Math.min(nextIntervalEn, nextIntervalSt + 1 + adjMaxWindow)),
                                        execInterval + 1,
                        ...
                        subIntervalEnd = subIntervalStart + adjMaxWindow;
                        // logger.info("Skipped interval - no vertices scheduled. " + subIntervalStart +
                        // " -- "+ subIntervalEnd);
        ...
        // logger.info("Engine finished in: " + (System.currentTimeMillis() - startTime)
        // * 0.001 + " secs.");
    ...
    private void execUpdates(final GraphChiProgram<VertexDataType, EdgeDataType> program,
        ...
        if (Runtime.getRuntime().availableProcessors() == 1) {
            ...
            /*
             * Parallel updates. One thread for non-parallel safe updates, others updated in
             * parallel. This guarantees deterministic execution.
             */
            ...
            for (int thrId = 0; thrId < nWorkers; thrId++) {
                ...
                // public void run() {
                int thrupdates2 = 0;
                GraphChiContext threadContext2 = chiContext.clone(1 + myId);
                ...
                try {
                    ...
                    for (int i = chunkStart; i < end; i++) {
                        ...
                        if (vertex != null && vertex.parallelSafe) {
                            thrupdates2++;
                            program.update(vertex, threadContext2);
                        }
                    }
    ...
    protected int initVertices(int nvertices, int firstVertexId, ChiVertex<VertexDataType, EdgeDataType>[] vertices)
            throws IOException {
        ...
        int blockId = (vertexDataConverter != null
                ? vertexDataHandler.load(firstVertexId, firstVertexId + nvertices - 1)
                : -1);
        for (int j = 0; j < nvertices; j++) {
            ...
            ChiVertex<VertexDataType, EdgeDataType> v = new ChiVertex<VertexDataType, EdgeDataType>(j + firstVertexId,
                    degree);
    ...
    private void loadBeforeUpdates(int interval, final ChiVertex<VertexDataType, EdgeDataType>[] vertices,
            final MemoryShard<EdgeDataType> memShard, final int startVertex, final int endVertex) throws IOException {
        ...
        synchronized (terminationLock) {
            ...
            if (!disableInEdges) {
                try {
                    ...
                    // logger.info("Loading memory-shard finished." +
    ...
    class AutoLoaderTask implements Callable<IntervalData> {
        ...
        public IntervalData call() {
            try {
                ...
                return new IntervalData(new VertexInterval(interval.getFirstVertex(), lastVertex), vertices,
                        vertexBlockid, memShard, intervalNum);

            } catch (NoEdgesInIntervalException nie) {
                return new IntervalData(new VertexInterval(interval.getFirstVertex(), interval.getLastVertex()),
                        vertices, -1, memShard, intervalNum);
    ...
    private int determineNextWindow(int subIntervalStart, int maxVertex)
            throws IOException, NoEdgesInIntervalException {
    ...
    // pyuhala: avoids param serialization issues
    public void setVDataConverterFloat() {
        BytesToValueConverter<VertexDataType> converter = (BytesToValueConverter<VertexDataType>) new FloatConverter();
        this.setVertexDataConverter(converter);
    }

    /**
     * Sets the bytes->edge value converter object. If the object is null, then no
     * edge-values are read (only adjacency information).
     * 
    ...
    // pyuhala: avoids param serialization issues
    public void setEdataConverterFloat() {
        BytesToValueConverter<EdgeDataType> converter = (BytesToValueConverter<EdgeDataType>) new FloatConverter();
        this.setEdataConverter(converter);
     ...
     * See http://code.google.com/p/graphchi/wiki/IntroductionToGraphChi section
     * "Parallel Updates"
     * 
    ...
    /**
     * Configures the maximum number of vertices loaded at any time. Default is 20
     * million. Generally you should not needed to modify this.
     * 
     * @param maxWindow
     */
    ...
    /**
     * Enables use of static window size (without adjusting the number of vertices
     * loaded at any time based on the amount of available memory). Only for
     * advanced users!
     * 
    ...
    /**
     * Experimental feature that enables GraphChi to load data ahead. This works
     * only with onlyAdjacency-setting. DO NOT USE - NOT TESTED.
     * 
     * @param autoLoadNext
     */
    ...
    public void printTop(int n) throws Exception {
        int i = 0;
        VertexIdTranslate trans = this.getVertexIdTranslate();
        TreeSet<IdFloat> topn = Toplist.topListFloat(this.baseFilename, this.numVertices(), n);
        for (IdFloat vertexRank : topn) {
    ...
    /**
     * GraphChi uses internal vertex ids. To translate from the internal ids to the
     * ids used in the original graph, obtain VertexIdTranslate object by using this
     * method and call translater.backward(internalId)
     * 
     * @return
     */
