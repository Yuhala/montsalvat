...
public class VertexData<VertexDataType> {
    ...
    public VertexData(int nvertices, String baseFilename, BytesToValueConverter<VertexDataType> converter)
            throws IOException {
        this(nvertices, baseFilename, converter, true);
    }

    public VertexData(int nvertices, String baseFilename, BytesToValueConverter<VertexDataType> converter,
            boolean _sparse) throws IOException {
        ...
        if (sparse && !sparseDegreeFile.exists()) {
            sparse = false;
            // logger.info("Sparse vertex data was allowed but sparse degree file did not
            // exist using dense");
        }
        ...
        if (!sparse) {
            ...
            // logger.info("Vertex file [" + vertexfile.getAbsolutePath() + "] length: " +
            // vertexfile.length()
            // + ", nvertices=" + nvertices + ", expected size: " + expectedSize);
    ...
    public void releaseAndCommit(int firstVertex, int blockId) throws IOException {
        ...
        if (!sparse) {
            ...
            // //logger.info("Vertex data write: " + dataStart + " -- " + (dataStart +
            // data.length));
            ...
            synchronized (vertexDataFile) {
                ...
                for (int i = 0; i < index.length; i++) {
                    vertexDataFile.writeInt(Integer.reverseBytes(index[i])); // Note: when writing, the random access
                                                                             // file does not take byte order into
                                                                             // account!
