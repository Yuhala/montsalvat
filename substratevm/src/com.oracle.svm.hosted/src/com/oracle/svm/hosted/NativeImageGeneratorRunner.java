...
public class NativeImageGeneratorRunner implements ImageBuildTask {
    ...
    private int buildImage(String[] arguments, ImageClassLoader classLoader) {
        ...
        String security_attrib = imageKind.is_secure ? "secure object" : "unsecure object";
         System.out.println("Image kind: " + imageKind + " " + security_attrib);
                    /* Check security type option */
            boolean secure = SubstrateOptions.SgxIn.getValue(parsedHostedOptions);
            boolean unsecure = SubstrateOptions.SgxOut.getValue(parsedHostedOptions);

            if (secure && unsecure) {
                throw UserError.abort("Cannot pass both option: "
                        + SubstrateOptionsParser.commandArgument(SubstrateOptions.SgxIn, "+") + " and "
                        + SubstrateOptionsParser.commandArgument(SubstrateOptions.SgxOut, "+"));
            } else if (secure) {
                System.out.println("Generating secure object -----------------------> pyuhala");
                imageKind.setSecurity(true);
                imageName = "main_in";
            } else if (unsecure) {
                System.out.println("Generating unsecure object -----------------------> pyuhala");
                imageKind.setSecurity(false);
                imageName = "main_out";

            } else {
                /* We will consider the default security type as unsecure */
                System.out.println(
                        "No sgx option specified: generating unsecure object by default -----------------------> pyuhala");
                imageKind.setSecurity(false);
                imageName = "main_out";
            }
            ...
            generator = new NativeImageGenerator(classLoader, optionParser, mainEntryPointData,imageKind.is_secure);
